...
export function usePixData(dateFrom?: string, dateTo?: string) {
  ...
  const fetchPixData = async () => {
    ...
    try {
      ...
      const fetchDayData = async (date: Date, index: number): Promise<PixBankData[]> => {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            if (response.status === 401 || response.status === 403) {
              console.warn(`🔒 Token expirado/inválido para ${dateStr}. Usando dados mockados.`)
              throw new Error('UNAUTHORIZED')
            }
            console.warn(`⚠️ Erro na requisição para ${dateStr}: status ${response.status}`)
            return []
          }
          ...
          if (responseText.trim() === 'Pesquisa não retornou dados') {
            console.log(`📭 ${dateStr}: Sem dados (API: "${responseText.trim()}")`)
            return []
          }
          
          // Verificar se a resposta parece ser JSON
          if (!responseText.trim().startsWith('[') && !responseText.trim().startsWith('{')) {
            console.error(`❌ Resposta não é JSON para ${dateStr}. Primeiros 200 chars:`, responseText.substring(0, 200))
            return []
          }
      ...
      const successfulRequests = allResults.filter(result => result.length > 0).length
      const emptyRequests = totalRequests - successfulRequests
      
      console.log(`📊 Resumo das requisições: ${successfulRequests}/${totalRequests} com dados, ${emptyRequests} vazias`)
      console.log(`📋 Total de registros brutos retornados: ${combinedData.length}`)
      
      // Debug: mostrar alguns registros brutos
      if (combinedData.length > 0) {
        console.log(`📝 Primeiros 3 registros brutos:`, combinedData.slice(0, 3))
      }
      ...
      console.log(`🎯 Dados filtrados: ${filteredData.length} registros com transações (de ${combinedData.length} totais)`)
      console.log(`📅 Dias/bancos com dados:`, filteredData.map(item => `${item.f110_date} - ${item.bank_num} (${item.total_count} transações)`))
      
      // Processar dados e corrigir URLs das imagens (já feito na fetchDayData, mas garantindo)
      const processedResult = filteredData.map(item => ({
        ...item,
        bank_image_url: item.bank_image_url.replace(/\\\//g, '/'),
        // Garantir que os percentuais estão corretos
        active_percent: item.total_amount > 0 ? (item.active_amount / item.total_amount) * 100 : 0,
        expired_percent: item.total_amount > 0 ? (item.expired_amount / item.total_amount) * 100 : 0,
        paid_percent: item.total_amount > 0 ? (item.paid_amount / item.total_amount) * 100 : 0
      }))
      ...
      if (err instanceof Error && err.message === 'UNAUTHORIZED') {
        ...
        const mockData: PixBankData[] = [
          {
            bank_num: '341',
            bank_image_url: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/chrome/chrome-original.svg',
            f110_date: defaultDateFrom,
            active_amount: 15000.50,
            expired_amount: 2500.00,
            paid_amount: 8750.25,
            active_count: 25,
            expired_count: 5,
            paid_count: 15,
          ...
          },
          {
            ...
            bank_image_url: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/chrome/chrome-original.svg',
            f110_date: defaultDateFrom,
            active_amount: 12000.00,
            expired_amount: 1800.00,
            paid_amount: 6900.75,
            active_count: 20,
            expired_count: 3,
            paid_count: 12,
          ...
          },
          {
            ...
            bank_image_url: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/chrome/chrome-original.svg',
            f110_date: defaultDateFrom,
            active_amount: 18500.00,
            expired_amount: 3200.50,
            paid_amount: 9800.25,
            active_count: 30,
            expired_count: 8,
            paid_count: 18,
